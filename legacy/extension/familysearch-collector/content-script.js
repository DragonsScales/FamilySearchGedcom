...
chrome.runtime.onMessage.addListener((message, _sender, sendResponse) => {
  if (message?.type !== 'FS_CAPTURE_PAGE') return false;
...
function captureVisibleFamilySearchPage() {
...
function extractFamilySearchIds(value) {
  return String(value ?? '').match(/\b[A-Z0-9]{4}-[A-Z0-9]{3}\b/g)?.map((id) => id.toUpperCase()) ?? [];
}
